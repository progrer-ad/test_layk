...
import { Box, Typography, Snackbar, Alert, useTheme, useMediaQuery, CssBaseline, CircularProgress } from '@mui/material';
...
import api from '@/util/api'; // Sizning Axios instance'ingiz
...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
    ...
    // MUHIM O'ZGARTIRISH: Tokenni localStorage'dan olish qismini olib tashlash
    // va to'g'ridan-to'g'ri `api` instance'idan foydalanish
    const fetchUser = useCallback(async () => {
        ...
        try {
            ...
            setUser(apiUser);
            localStorage.setItem('user', JSON.stringify(apiUser)); // UI uchun user ma'lumotini saqlash xavfsiz

        } catch (err: any) {
            if (axios.isAxiosError(err) && err.response?.status === 401) {
                // Agar 401 Unauthorized xatosi bo'lsa, login sahifasiga yo'naltiramiz
            ...
            localStorage.removeItem('user'); // noto'g'ri foydalanuvchi ma'lumotini o'chirish
        } finally {
            setTimeout(() => {
                setLoading(false);
                setShowLoader(false);
    ...
    const handleLogout = async () => {
        try {
            // Logout so'rovini yuborish. Brauzer HTTP-Only cookie'ni avtomatik qo'shadi.
            ...
            // Logout muvaffaqiyatli bo'ladimi yoki yo'qmi, baribir lokal ma'lumotlarni o'chirish
    ...
    if (loading) {
        return (
            ...
            >
                <CircularProgress size={80} sx={{ color: colors.loaderOrbital }} />
                <Typography
                    variant="h4"
                    component="h1"
                    sx={{
    ...
    if (!user) {
        return null; // Yo'naltirishdan oldin bo'sh sahifa qaytarish
    }
